...
class GaussianModel:
    ...
    def __init__(self, sh_degree, optimizer_type="default"):
        ...
        # Canonical pose caches (filled once geometry is instantiated).
        self._xyz0: Optional[torch.Tensor] = None
        self._rotation0: Optional[torch.Tensor] = None
        self._scaling0: Optional[torch.Tensor] = None
        ...
        if self.isotropic:
            ...
            if scaling_tensor.shape[0] == 0:
                ...
                new_scaling = self.scaling_inverse_activation(
                    scaling_tensor[:, 0].unsqueeze(-1).repeat(N, 1) / (0.8 * N)
                )
    ...
    # ------------------------------------------------------------------
    # Canonical pose helpers for LBS-based pipelines
    # ------------------------------------------------------------------
    @torch.no_grad()
    def snapshot_canonical_pose(self) -> None:
        """
        Store the current geometry as the canonical pose.
        """

        if self._xyz.numel() == 0:
            return
        self._xyz0 = self._xyz.detach().clone()
        self._rotation0 = self._rotation.detach().clone()
        self._scaling0 = self._scaling.detach().clone()
